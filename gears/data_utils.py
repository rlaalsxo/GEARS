...
def get_DE_genes(adata, skip_calc_de):
    ...
    adata.obs['percent_mt'] = (adata[:, adata.var['mt']].X.sum(1) / adata.X.sum(1)) * 100
    adata = adata[adata.obs['percent_mt'] < 20, :]

    total_cells = adata.shape[0]  # adata의 세로 길이 = 총 세포 수

    min_cell_percentage = 0.01 
    min_cells_threshold = int(total_cells * min_cell_percentage)  # 비율 → 절대값 변환
...
def get_dropout_non_zero_genes(adata):
    ...
    # 병렬 처리를 위한 내부 함수: 각 perturbation에 대해 계산 수행
    def process_pert(pert):
        # pert: adata.uns['rank_genes_groups_cov_all']의 key (예: 'GeneX+ctrl' 등)
        p = pert_full_id2pert[pert]
        # 해당 perturbation의 셀들에 대한 평균 발현량 계산
        ...
        return pert, np.sort(non_zero), np.sort(non_dropouts), \
               np.array(non_dropout_20_gene_id), np.array(non_zero_20_gene_id)

    # 병렬 처리를 위한 ThreadPoolExecutor 사용 (Windows, Linux 모두 동일하게 동작)
    from concurrent.futures import ThreadPoolExecutor, as_completed
    perturbations = list(adata.uns['rank_genes_groups_cov_all'].keys())
    with ThreadPoolExecutor(max_workers=None) as executor:
        futures = {executor.submit(process_pert, pert): pert for pert in perturbations}
        for future in as_completed(futures):
            pert, sorted_non_zero, sorted_non_dropout, non_dropout_20_gene_id, non_zero_20_gene_id = future.result()
            non_zeros_gene_idx[pert] = sorted_non_zero
            non_dropout_gene_idx[pert] = sorted_non_dropout
