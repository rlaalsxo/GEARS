...
class GEARS:
    ...
    def load_pretrained(self, path):
        ...
        config.pop('num_genes', None)
        config.pop('num_perts', None)
        ...
        print("Saving model - First layer weights (before saving):")
        first_layer_weights = list(self.best_model.state_dict().values())[0]
    ...
    def plot_perturbation(self, query, save_file = None, model=None, box_plot = False):
        ...
        if self.config['uncertainty']:
            pred, unc = self.predict([query_], model=model, box_plot=box_plot)
        else:
            pred = self.predict([query_], model=model, box_plot=box_plot)
        ...
        if box_plot:
            ...
            sns.violinplot(data=pred, inner="quartile", color="red")
        else:
    ...
    def train(self, predict_perturbation,
        ...
        for epoch in range(epochs):
            ...
            val_res = evaluate(val_loader, self.model,
                                 self.config['uncertainty'], self.device)
            train_metrics, _ = compute_metrics(train_res, save_csv=True, csv_filename=f"{self.model_path}/train_pert_mse.csv")
            val_metrics, _ = compute_metrics(val_res, save_csv=True, csv_filename=f"{self.model_path}/val_pert_mse.csv")
            ...
            csv_filename = f"{self.model_path}/Overall_MSE.csv"

            log_entry = [train_metrics['mse'], val_metrics['mse']]
            ...
            with open(csv_filename, "a", newline="", encoding="utf-8") as file:
                ...
                if not file_exists:
                    writer.writerow(["Train Overall MSE", "Validation Overall MSE"])
            ...
            csv_filename = f"{self.model_path}/Top20DEG_MSE.csv"

            log_entry = [train_metrics['mse_de'], val_metrics['mse_de']]
            ...
            with open(csv_filename, "a", newline="", encoding="utf-8") as file:
                ...
                if not file_exists:
                    writer.writerow(["Train MSE (Top 20 DE)", "Validation MSE (Top 20 DE)"])
