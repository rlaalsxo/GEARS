...
class PertData:
    ...
    def prepare_split(self, split = 'simulation', 
        ...
        if split == 'simulation':
            ...
            train_conditions = self.set2conditions['train']
            df = pd.DataFrame(train_conditions, columns=['train_conditions'])
            df.to_csv(f'{self.model_path}/train_conditions.csv', index=False, encoding='utf-8-sig')
            for i,j in subgroup['test_subgroup'].items():
                print_sys(i + ':' + str(len(j)))
            df = pd.DataFrame(dict([(k, pd.Series(v)) for k, v in subgroup['test_subgroup'].items()]))
            df.to_csv(f"{self.model_path}/test_subgroup.csv", index=False)
        print_sys("Done!")
    ...
    def create_cell_graph_dataset(self, split_adata, pert_category, num_samples=1):
        ...
        for i in range(0, n_obs, chunk_size):
            chunk = adata_[i: i + chunk_size]
            Xs = []
            ys = []
            # pert_category가 ctrl이 아닐 경우, 각 pert cell에 대해 control 셀들을 매칭
            if pert_category != 'ctrl':
                for cell_z in chunk.X:
                    # control 셀 중 num_samples개를 랜덤하게 선택
                    ctrl_samples = self.ctrl_adata[np.random.randint(0, len(self.ctrl_adata), num_samples), :]
                    # 선택된 control 샘플과 현재 pert cell을 pair로 저장
                    for c in ctrl_samples.X:
                        Xs.append(c)
                        ys.append(cell_z)
            else:
                # ctrl인 경우, 각 셀에 대해 자기 자신을 사용
                for cell_z in chunk.X:
                    Xs.append(cell_z)
                    ys.append(cell_z)

            # Xs, ys는 각 청크에서 생성된 (control, pert) 쌍 리스트
            for X, y in zip(Xs, ys):
                # X와 y가 희소행렬인 경우 toarray() 호출 (이미 dense면 그대로 사용)
                X_arr = X.toarray() if hasattr(X, "toarray") else X
                y_arr = y.toarray() if hasattr(y, "toarray") else y
                cell_graphs.append(self.create_cell_graph(X_arr, y_arr, de_idx, pert_category, pert_idx))
            # 각 청크 처리 후 필요시 가비지 컬렉션 실행
            import gc
            gc.collect()
    ...
    def create_dataset_file(self):
        ...
        with ThreadPoolExecutor(max_workers=None) as executor:
            # 각 조건에 대해 create_cell_graph_dataset()를 병렬로 실행
            future_to_condition = {
                executor.submit(self.create_cell_graph_dataset, self.adata, cond): cond
                for cond in conditions
            }
            for future in as_completed(future_to_condition):
                cond = future_to_condition[future]
                try:
                    dataset = future.result()
                except Exception as exc:
                    print_sys(f"Condition {cond} generated an exception: {exc}")
                    dataset = []
                results[cond] = dataset
        self.dataset_processed = results
        print_sys("Done!")

    def MyfileLoad(self, data_name ,adata_path, dataset_fname):
        data_name = data_name.lower()
        self.dataset_path = os.path.join(self.data_path, data_name)
        self.dataset_name = data_name
        self.adata = sc.read_h5ad(adata_path)
        # self.adata = get_DE_genes(adata, False)
        # self.adata = get_dropout_non_zero_genes(self.adata)
