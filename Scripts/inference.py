...
def compute_metrics(results, save_csv=False, csv_filename="perturbation_mse.csv"):
    """
    Given results from a model run and the ground truth, compute metrics.
    Optionally, save per-perturbation MSE values to a CSV file (append mode).
    """
    metrics = {}
    metrics_pert = {}
    ...
    for pert in np.unique(results['pert_cat']):
        ...
        for m, fct in metric2fct.items():
            ...
            # Save perturbation-specific MSE
            if m == 'mse':
                per_pert_mse.append([pert, val])
    ...
    if save_csv:
        ...
        with open(csv_filename, "a", newline="", encoding="utf-8") as file:
            ...
            # 파일이 없으면 perturbation을 헤더로 추가
            if not file_exists:
                header = [pert for pert, _ in per_pert_mse]  # perturbation 이름들을 헤더로
                writer.writerow(header)

            # perturbation별 MSE 값들만 한 줄(row)로 추가
            log_entry = [mse for _, mse in per_pert_mse]
            writer.writerow(log_entry)

        print(f"Perturbation-wise MSE saved to {csv_filename}")
